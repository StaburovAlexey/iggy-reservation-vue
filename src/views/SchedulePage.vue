              ...
              <template #date-cell="{ data }">
                <div class="schedule-date" :class="{
                  ...
                  <div class="schedule-date__header">
                    ...
                    <el-tag v-if="hasAssignments(data.day) || isCompactView" size="small" effect="plain" type="info"
                      round>
                      {{ totalAssignments(data.day) }}
                    </el-tag>
                  </div>

                  <div v-if="!isCompactView" class="schedule-date__groups">
                    <div v-for="group in previewGroups(data.day)" :key="group.key" class="schedule-date__group"
                      ...
                      <div class="schedule-date__group-name">
                        {{ group.label }}
                      </div>

                      <div class="schedule-date__people">
                        <el-tag v-for="person in group.people.slice(0, 2)" :key="person.id" :type="group.tag"
                          size="small" round :closable="isAdmin" @close.stop="
                            removePersonFromDate(data.day, group.key, person.id)
                            ">
                          <div class="schedule-date__person">
                            ...
                            <span class="schedule-date__person-name">{{
                              person.name
                            }}</span>
                          </div>
                        </el-tag>
        ...
        <el-card v-if="isAdmin" class="schedule-grid__employees" shadow="hover">
          <div class="employee-header">
            ...
            <p class="employee-hint">
              Перетащите сотрудника на нужную группу внутри дня.
            </p>
          </div>
          ...
          <div class="employee-list">
            <el-tag v-for="emp in employees" :key="emp.id" class="employee-pill" effect="plain" type="info"
              ...
              <div class="employee-pill__content">
                <el-avatar size="small" :src="emp.avatar" class="employee-pill__avatar">
...
const updateCompactFlag = () => {
  ...
  isCompactView.value = window.innerWidth < 900;
};
...
const getEmployeeAvatarById = (id) => {
  ...
  const found = (employees.value || []).find((emp) => emp?.uuid === id);
...
const normalizePeople = (list = [], groupKey = "") => {
  ...
  return list
    ...
    .map((raw, index) => {
      ...
      const name =
        item.name || item.login || (typeof item === "string" ? item : "");
...
const applySchedule = (payload) => {
  ...
  (Array.isArray(raw) ? raw : []).forEach((item) => {
    const dateValue =
      item?.date || item?.day || item?.date_key || item?.dateKey;
    ...
    next[key] = {
      opening: normalizePeople(
        item.opening || item.open || item.morning,
        "opening"
      ),

      closing: normalizePeople(
        item.closing || item.close || item.evening,
        "closing"
      ),
...
const shiftMonth = (delta) =>
  updateMonth(currentMonth.value.add(delta, "month"), { syncValue: true });
...
const normalizeDroppedEmployee = (rawEmployee) => {
  ...
  const avatar =
    employee.avatar || resolveAvatar(employee) || getEmployeeAvatarById(id);
...
const removePersonFromDate = async (date, groupKey, personId) => {
  ...
  const updatedGroup = (dayEntry[groupKey] || []).filter(
...
.schedule-date.is-compact .schedule-date__day {
  font-size: 14px;
}
...
@media (max-width: 900px) {
  ...
  .schedule-card__scroll :deep(.el-calendar__body) {
    padding: 0;
  }

  .schedule-card__scroll :deep(.el-calendar-table td) {
    padding: 2px;
  }

  .schedule-card__scroll :deep(.el-calendar-table .el-calendar-day) {
    padding: 4px;
    height: 30px;
  }
