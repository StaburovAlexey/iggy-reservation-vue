            ...
            <template #date-cell="{ data }">
              ...
              >
                ...
                <div class="schedule-date__groups">
                  <div
                    v-for="group in previewGroups(data.day)"
                    :key="group.key"
                    class="schedule-date__group"
                    @dragover.prevent
                    @drop="handleGroupDrop(data.day, group.key)"
                  >
                    <div class="schedule-date__group-name">{{ group.label }}</div>
                    <div class="schedule-date__people">
                      <el-tag
                        v-for="person in group.people.slice(0, 2)"
                        :key="person.id"
                        :type="group.tag"
                        size="small"
                        round
...
const serializeGroupForSave = (list = []) =>
  (list || [])
    .map((item) => {
      if (!item) return null;
      const name = String(item.name || item.full_name || item.title || item.login || item).trim();
      if (!name) return null;
      const id = item.id || item.uuid || item.user_id || item.login || null;
...
const normalizeDroppedEmployee = (employee) => {
  if (!employee) return null;
  const name = employee.name || employee.full_name || employee.title || employee.login || "";
  const cleaned = String(name || "").trim();
  if (!cleaned) return null;
